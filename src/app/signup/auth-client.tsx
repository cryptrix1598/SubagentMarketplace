...
import { Package, Github, Mail, Eye, EyeOff, ArrowRight, Loader2 } from "lucide-react";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        ...
        if (result.error) {
          toast({ title: "Error", description: result.error.message || "Invalid credentials", variant: "destructive" });
        } else {
          router.push("/dashboard");
          router.refresh();
        }
      ...
      >
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-muted-foreground">
            {isSignUp
              ? "Start publishing and discovering Claude Code agents."
              : "Sign in to your Claude Agent Hub account."}
          </p>
        </div>
